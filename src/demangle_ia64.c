 ...
 *
 * The implementation in this file is adapted from demangle.cc in the glog
 * project. See LICENSE for details.
 */

/**
 * Itanium C++ ABI symbol demanging.
 *
 * This only supports Linux right now.
 */
...
static const AbbrevPair operator_list[] = {
  {"nw", "new"},
  {"na", "new[]"},
  {"dl", "delete"},
  {"da", "delete[]"},
...
static const AbbrevPair builtin_type_list[] = {
  {"v", "void"},
  {"w", "wchar_t"},
  {"b", "bool"},
  {"c", "char"},
...
static const AbbrevPair substitution_list[] = {
  {"St", ""},
  {"Sa", "allocator"},
  {"Sb", "basic_string"},
...
static bool ParseOperatorName(State* state) {
  ...
  *state = copy;
  ...
  const AbbrevPair *p;
  for (p = operator_list; p->abbrev != NULL; ++p) {
...
static bool ParseBuiltinType(State* state) {
  const AbbrevPair *p;
  for (p = builtin_type_list; p->abbrev != NULL; ++p) {
    if (state->mangled_cur[0] == p->abbrev[0]) {
      MaybeAppend(state, p->real_name);
      ++state->mangled_cur;
      return true;
...
static bool ParseSubstitution(State* state) {
  ...
  *state = copy;
  ...
  if (ParseOneCharToken(state, 'S')) {
    const AbbrevPair *p;
    for (p = substitution_list; p->abbrev != NULL; ++p) {
...
#endif  // SHBT_USE_BUILTIN_IA64_DEMANGLER
